...
class Distance_Generator(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.l11 = CBR(in_channel=c1, out_channel=c1, kernel=7, padding=3,
                       use_norm=self.use_norm, lrelu_use=self.lrelu_use, batch_mode=self.batch_mode)  # 7, 3
        ...
        self.l31 = CBR(in_channel=c2, out_channel=c2, kernel=5, padding=2,
                       use_norm=self.use_norm, lrelu_use=self.lrelu_use, batch_mode=self.batch_mode)  # 5, 2
        ...
        self.l51 = CBR(in_channel=c3, out_channel=c3, kernel=3, padding=1,
                       use_norm=self.use_norm, lrelu_use=self.lrelu_use, batch_mode=self.batch_mode)  # 3, 1

        # output
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)
        # self.conv_out_d = nn.Conv2d(in_channels=c3, out_channels=1, kernel_size=1)
        self.conv_out_d = nn.Conv2d(in_channels=c1*5, out_channels=1, kernel_size=1)
        self.aspp = aspp(args)
    ...
    def forward(self, x):
        # x: 256x256
        l1 = self.mpool0(self.l11(self.l10(x)))  # 128x128
        l2 = self.mpool0(self.l21(self.l20(l1))) # 64x64
        l3 = self.mpool0(self.l31(self.l30(l2))) # 32x32
        l4 = self.mpool0(self.l41(self.l40(l3))) # 16x16
        l5 = self.mpool0(self.l51(self.l50(l4))) # 8x8
...
class aspp(nn.Module):
    def __init__(self, args):
        ...
        c_in = args.initial_channel*4
        c_out = args.initial_channel
        ...
        self.module_list.append(CBR(in_channel=c_in, out_channel=c_out, use_norm=self.use_norm, batch_mode=self.batch_mode,
                                    kernel=1, stride=1, padding=0, lrelu_use=self.lrelu_use))
        for r in atrous_rates:
            self.module_list.append(CBR(in_channel=c_in, out_channel=c_out, use_norm=self.use_norm, batch_mode=self.batch_mode,
                                    kernel=3, stride=1, padding=r, rate=r))

        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.conv = CBR(in_channel=c_in, out_channel=c_out, use_norm=self.use_norm, batch_mode=self.batch_mode,
                                    kernel=1, stride=1, padding=0, lrelu_use=self.lrelu_use)
    ...
    def forward(self, x):
        ...
        x_out.append(self.pooling(x))

        return torch.cat(x_out, dim=1)

class Distance_Generator_original(nn.Module):

    def __init__(self, args):
        super(Distance_Generator_original, self).__init__()
        self.input_channel = 1
        self.output_channel = 1
        self.lrelu_use = args.lrelu_use
        self.use_norm = True
...
class switchable_cycle(nn.Module):
    ...
    def __init__(self, args, input_channel=1, output_channel=2):
        ...
        self.l11 = CBR(in_channel=c1, out_channel=c1, use_norm=self.use_norm, lrelu_use=self.lrelu_use)
        ...
        [self.l7_code_gm, self.l7_code_gs] = self.build_code_g(c2)
        [self.l8_code_gm, self.l8_code_gs] = self.build_code_g(c1)
    ...
    def forward_decoder(self, x, skip, distance_code):
        ...
        l5 = self.conv_T5(self.forward_code_g(self.l51(x), distance_code, self.l5_code_gm, self.l5_code_gs))

        # l6 = l5
        l6 = self.l61(torch.cat([l5, skip[0]], dim=1))
        l6 = self.conv_T6(self.forward_code_g(self.l60(l6), distance_code, self.l6_code_gm, self.l6_code_gs))
        # l6 = self.l60(self.forward_code_g(l6, distance_code, self.l6_code_gm, self.l6_code_gs))

        # l7 = l6
        l7 = self.l71(torch.cat([l6, skip[1]], dim=1))
        l7 = self.conv_T7(self.forward_code_g(self.l70(l7), distance_code, self.l7_code_gm, self.l7_code_gs))

        # l8 = l7
        l8 = self.l81(torch.cat([l7, skip[2]], dim=1))
        l8 = self.conv_T8(self.forward_code_g(self.l80(l8), distance_code, self.l8_code_gm, self.l8_code_gs))
        # l8 =self.l80(self.forward_code_g(l8, distance_code, self.l8_code_gm, self.l8_code_gs))

        # l9 = l8
        l9 = self.l91(torch.cat([l8, skip[3]], dim=1))
        out = self.forward_code_g(self.l90(l9), distance_code, self.l9_code_gm, self.l9_code_gs)
    ...
    def forward(self, x, d_true, d_trans, train=True):
        ...
        d_true_vec = torch.cat([torch.randn(size=[b, self.z_dim - 1]).to(d_true.device), d_true*10], dim=1)
        ...
        d_trans_vec = torch.cat([torch.randn(size=[b, self.z_dim - 1]).to(d_trans.device), d_trans*10], dim=1)
        d_trans_code = self.shared_code_generator(d_trans_vec)

        # identity reconstruction
        skip, encoded = self.forward_encoder(x, d_true_code)
