...
def ASM_torch_ver(O, lamb, d, px, requires_grad=True, zero_padding=False, pixel_diff=False): # torch version
    ...
    G_in = 1 - (lamb**2)*(torch.bmm(fx**2, torch.ones(size=[batch, 1, fx.shape[1]],dtype=torch.float).to(device))
                          + torch.bmm(torch.ones(size=[batch, fy.shape[2], 1],dtype=torch.float).to(device), fy**2))
    G_in = (torch.sqrt((G_in>0)*1*G_in)/lamb).view(batch, 1, fx.shape[1], fy.shape[2])

    G_in.requires_grad_(True)
    # G_in = torch.exp(1j*2*pi*d*G_in)
    G_in = torch.exp(1j*2*pi*d.repeat(1,1,G_in.shape[-2], G_in.shape[-1])*G_in)
